...
{
    public class bl_CarAudio : bl_MonoBehaviour
    {
        ...
        private bl_CarController m_CarController; // Reference to car we are controlling
        ...
        void CheckDistances()
        {
            if (bl_GameManager.Instance == null || bl_GameManager.Instance.CameraRendered == null)
                return;

            Distance = (bl_GameManager.Instance.CameraRendered.transform.position - m_Transform.localPosition).sqrMagnitude;
            // stop sound if the object is beyond the maximum roll off distance
            if (m_StartedSound && Distance >= maxRolloffDistance * maxRolloffDistance)
            {
                StopSound();
            }
        ...
        {         
            if (m_StartedSound)
            {
                ...
                else if(engineSoundStyle == EngineAudioOptions.Curve)
                {
                    float p = m_CarController.CurrentSpeed / m_CarController.MaxSpeed;
                    float r = pitchCurve.Evaluate(p);
                    m_HighAccel.pitch = r * pitchMultiplier;
                    m_HighAccel.dopplerLevel = useDoppler ? dopplerLevel : 0;
                    m_HighAccel.volume = 1;
                }
                else
