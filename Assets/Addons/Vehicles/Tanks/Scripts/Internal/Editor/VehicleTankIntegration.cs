...
public class VehicleTankIntegration : AddonIntegrationWizard
{
    //REQUIRED
    ...
    public override void OnEnable()
    {
        ...
        MFPSAddonsInfo addonInfo = MFPSAddonsData.Instance.Addons.Find(x => x.NiceName == "Vehicle: Tank");
    ...
    public override void DrawWindow(int stepID)
    {
        ...
        else if (stepID == 3)
        {
            ...
            if (!canDoit)
            {
                var tankUI = bl_VehicleUI.Instance.GetComponentInChildren<bl_TankStateUI>(true);
                if(tankUI == null)
                {
                    canDoit = true;
                }
        ...
        else if (stepID == 4)
        {
            ...
            if (MFPSEditorStyles.ButtonOutline("Tank", BLUE_COLOR, GUILayout.Width(150), GUILayout.Height(24)))
            {
    ...
    private void IntegrateInMap()
    {
        ...
        if (script != null)
        {
            ...
            if (tankUI != null)
            {
                Debug.Log($"{ADDON_NAME} has been already integrated in this scene!");
                return;
            }
        }
        ...
        var tankUI2 = bl_VehicleUI.Instance.GetComponentInChildren<bl_TankStateUI>(true);
        if(tankUI2 == null)
        {
            var tankUIInstance = InstancePrefab("Assets/Addons/Vehicles/Tanks/Prefabs/UI/TankUI.prefab");
            tankUIInstance.transform.SetParent(bl_VehicleUI.Instance.transform, false);
            EditorUtility.SetDirty(tankUIInstance);
        }
...
#if !MFPS_VEHICLE
    [MenuItem("MFPS/Addons/Vehicle/Tank/Enable")]
...
#if MFPS_VEHICLE
    [MenuItem("MFPS/Addons/Vehicle/Tank/Disable")]
    ...
    [MenuItem("MFPS/Addons/Vehicle/Tank/Integrate")]
    static void Open()
    {
