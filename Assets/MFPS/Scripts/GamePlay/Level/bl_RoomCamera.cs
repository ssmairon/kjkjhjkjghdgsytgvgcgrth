...
public class bl_RoomCamera : bl_RoomCameraBase
{
...
#if MFPSM
        bl_TouchHelper.Instance?.SetMobileCanvasVisible(cameraMode == CameraMode.Spectator);
#endif
    }
    ...
    void FlyMovement()
    {
        ...
        float mouseX = bl_GameInput.MouseX;
        float mouseY = bl_GameInput.MouseY;
        float moveV = bl_GameInput.Vertical;
        float moveH = bl_GameInput.Horizontal;

#if MFPSM
        if (bl_UtilityHelper.isMobile && bl_TouchPad.Instance != null)
        {
            Vector2 minput = bl_TouchPad.Instance.GetInput(cameraSensitivity * 0.1f);
            mouseX = minput.x;
            mouseY = minput.y;
        ...
        {
            m_Transform.position += m_Transform.forward * (normalMoveSpeed * fastMoveFactor) * moveV * Time.deltaTime;
            m_Transform.position += m_Transform.right * (normalMoveSpeed * fastMoveFactor) * moveH * Time.deltaTime;
        }
        else if (Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.RightControl))
        {
            m_Transform.position += m_Transform.forward * (normalMoveSpeed * slowMoveFactor) * moveV * Time.deltaTime;
            m_Transform.position += m_Transform.right * (normalMoveSpeed * slowMoveFactor) * moveH * Time.deltaTime;
        }
        else
        {
            m_Transform.position += m_Transform.forward * normalMoveSpeed * moveV * Time.deltaTime;
            m_Transform.position += m_Transform.right * normalMoveSpeed * moveH * Time.deltaTime;
        }
