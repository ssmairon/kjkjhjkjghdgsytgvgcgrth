...
public class AddWeaponTutorial : TutorialWizard
{
    ...
    private readonly GifData[] AnimatedImages = new GifData[]
   {
    ...
    void DrawFPWeapon()
    {
        ...
        {
            DrawText("Now, select the top of the duplicated weapon <i>(where bl_Gun is attached)</i> and click once in the <b>FirePoint</b> value <i>(not the property name but the field value)</i>. With this, the hierarchy will fold out and automatically select the <b>FirePoint</b> object that also contains the muzzle flash effect inside it; it will be located inside the old weapon model, so with the <b>FirePoint</b> object selected, drag it in the hierarchy window and put it inside your new weapon model specifically as the child of your weapon barrel transform.\n \nThen adjust the position and rotation of the FirePoint and Muzzle Flash effect so it is placed precisely at the end of your weapon model barrel or wherever the bullet should be fired; once you do that, delete the old model from the hierarchy.");
            DownArrow();
            DrawAnimatedImage(3);
            DrawNote("Since MFPS 1.9 the muzzle flash and cartridge effect are located inside the Fire Point object so you only have to select the FirePoint object at difference of the video above where the 3 objects are selected and re-parented separately.");
        }
        else if (subStep == 5)
        {
            DrawText("Select the top of new weapon model where 'Animation' or 'Animator' component is attached and add the script <b>'bl_WeaponAnimation'</b> (click the inspector button 'Add Component' > write bl_WeaponAnimation and then click it).");
            DrawAnimatedImage(4);
            DownArrow();
            if (animationType == 0)
            {
